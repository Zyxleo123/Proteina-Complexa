...
### CPSea (cyclic peptide binders)

[CPSea](https://github.com/YZY010418/CPSea) provides cyclic peptide–receptor complexes. The **CPSea_PDB** subset (~12k experimental structures) lives on scratch at `$CPSEA_DATA_PATH` (default: `/zfsauton/scratch/yixiz/CPSea/CPSea_PDB`). Adapt it for binder training via [`script_utils/preprocess_cpsea.py`](script_utils/preprocess_cpsea.py):

```bash
# Raw structures (extract once): $CPSEA_DATA_PATH/CPSea_PDB_pdb/*.pdb.gz
python script_utils/preprocess_cpsea.py \
  --pdb-root $CPSEA_DATA_PATH/CPSea_PDB_pdb \
  --index $CPSEA_DATA_PATH/CPSea_PDB_index.txt \
  --cluster-tsv $CPSEA_DATA_PATH/CPSea_PDB_properties/CPSea_PDB_Cluster.tsv \
  --out-dir $CPSEA_DATA_PATH/preprocessed \
  --seed 42
```

Preprocessing:
- Renames chains **R→A** (target), **L→B** (binder)
- **Intended removals:** hydrogens, ACE/NME caps (logged per structure)
- **Preserves:** cyclization `CONECT` records (remapped atom serials; ignored by training loaders)
- **Reproducible:** cluster-based train/val/test splits (`--seed`); manifests under `{out_dir}/manifest/`

| Column | Description |
|--------|-------------|
| `example_id` | CPSea structure stem |
| `path` | Absolute path to cleaned `.pdb` |
| `binder_chain_id` | Always `"B"` after preprocessing |
| `cluster_id` | Foldseek cluster center (for splits) |
| `peptide_length` | Binder chain residue count |
| `cyclization_type` | `disulfide`, `head_tail`, or `other` (from remapped CONECT) |

Training config: [`configs/dataset/unified/cpsea_peptide.yaml`](configs/dataset/unified/cpsea_peptide.yaml) reads metadata from `$CPSEA_DATA_PATH/preprocessed/metadata/`. Audit outputs: `$CPSEA_DATA_PATH/preprocessed/manifest/`.

**Smoke training** (single GPU, 20 steps, finetune from `./ckpts/complexa.ckpt`):
...
Use the 100-sample override (before scratch preprocess exists) as documented in [`configs/example/training_cpsea_peptide_smoke.yaml`](configs/example/training_cpsea_peptide_smoke.yaml).

**From-scratch smoke** (random flow-model init; still uses frozen `./ckpts/complexa_ae.ckpt` for sidechain latents):
